    ...
    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
        ...
        @foreach($orders as $order)
            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                ...
                <td class="w-4 p-4">
                    <form action="{{ route('restaurant.new.order.update', $order->id) }}" method="post" id="form-delivery-status-submit-{{$c}}">
                        @csrf
                        <select name="delivery_status" value="delivery_status" class="text-[#374151] text-[.7rem] xs:text-[.9rem] font-semibold border-[#d1d5db] rounded-[.4rem] py-[.15rem] xs:py-[.25rem] shadow-sm px-[.55rem] xs:px-[.75rem] mr-[.25rem] w-fit" id="select_change" onchange="submitDeliveryStatus(<?php echo $c; ?>)">
                            <option value="{{$order->delivery_status}}" selected>New Order</option>
