    ...
    <div class="grid grid-cols-1 min-[350px]:grid-cols-2 xl:grid-cols-3 items-center justify-center px-[5%] 2xl:px-[10vw] gap-y-[1.5rem] scroll-smooth">
        @if($products)
            @foreach($products as $product)
                <a href="{{ route('products.list', $product->id) }}" class="hover:scale-[1.02] group [perspective:1000px]">
        ...
        @else
            <div class="hover:scale-[1.02] group [perspective:1000px]">
                <!-- <div class="relative transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]"> -->
                    <div class="h-[9rem] sm:h-[16.8rem] md:h-[23.8rem]">

                        <div class="w-[8rem] sm:w-[13.8rem] md:w-[19.8rem]  rounded-[.7rem] bg-gradient-to-r from-[#f9f9f9]  via-[#ececec] to-[#f8f8f8] border-y-4 border-[#222] h-full flex items-center justify-center">
                                <h3 class="text-[1.2rem] sm:text-[1.5rem] text-[#222] truncate first-letter:capitalize">No food available</h3>
                            <!-- </div> -->
                        </div>
                    </div>
                <!-- </div> -->
